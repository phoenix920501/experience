...
# 各个数据集分别做Kmeans统计anchor
'''     args:datasetdir：数据集所在目录
             savedir:保存目录
        return:各个数据集9个anchor统计信息 txt文件
'''
class Box():
...
def compute_centroids(label_path,n_anchors,loss_convergence,grid_size,iterations_num,plus,cropsize,savepath):
    ...
    for line in lines:
            ...
            #print("temp:",temp)
            width=round(float(temp[5])*cropsize,3)
            height=round(float(temp[6])*cropsize,3)
...
datasetslist=os.listdir(datasetdir)
    ...
    for i in range(len(datasetslist)):
            ...
            savepath=os.path.join(savedir,datasetname)
            print("savepath:",savepath)
            compute_centroids(datapath,n_anchors,loss_convergence,grid_size,iterations_num,plus,cropsize,savepath)